...
module.exports = async({ name }) => {
  ...
  let { tech, version, anthor } = await inquirer.prompt([{
    type: 'list',
  ...
  switch (tech.toLowerCase()) {
    case 'vue':
      return await require('./vue')({ name, version, anthor })
    case 'react':
      return await require('./react')({ name, version, anthor })
    default:
      return await require('./jquery-handlebars')({ name, version, anthor })
  }
}
